...
namespace Studing_Journal
{
    class WordDocument
    {
        ...
        public void Student_list()
        {
            ...
            string file_name = Registry_Class.DirPath + "\\СГ_" + Group_name + DateTime.Now.ToString("_hh_mm_ss_dd_MM_yyyy") + ".docx";
            ...
            try
            {
                ...
                word.Table tbStudents = document.Tables.Add(pTable.Range, table.Rows.Count + 1, table.Columns.Count);
                ...
                for (int i = 2; i <= tbStudents.Rows.Count; i++)
                    ...
                    {
                        tbStudents.Cell(i, j).Range.Text = table.Rows[i - 2][j - 1].ToString();
                    }
            }
            catch (Exception ex)
            {
