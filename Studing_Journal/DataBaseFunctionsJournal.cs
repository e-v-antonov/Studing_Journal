    ...
    class DataBaseFunctionsJournal
    {
        ...
        public void dtJouranlCreate()
        {
            ...
            try
            {
                ...
                table.Clear();
                for (int i = 0; i<dtJournal.Columns.Count; i++)
                    for (int j = 0; j<dtJournal.Rows.Count; j++)
                    {
                        command.CommandText = "select [dbo].[Students_Marks]('"
                            +dtJournal.Rows[i+1][0].ToString()+"','"
                            + dtJournal.Rows[0][j + 1].ToString() + "')";
                        dtJournal.Rows[i + 1][j + 1] = command.ExecuteScalar().ToString();
                    }
