...
namespace Studing_Journal
{
    class DataBaseView
    {
        ...
        string qrvProfiles_List = "select [Login_list],[Password_list], [Status_list]  from [dbo].[Profiles_List]",
            ...
            qrvGroup_academic_performance = "select [Group_name],[Grade_point_average],[Quality_percentage] from [dbo].[Group_academic_performance]",
        ...
        public void dtvDepartment_statistic_Groups()
        {
            SqlDependency dependency = new SqlDependency(command);
            SqlDependency.Start(Registry_Class.sqlConnection.ConnectionString);
            dependency.OnChange += onchangeDepartment_statistic_Groups;
            dtvFill(vdtDepartment_statistic_Groups, qrvDepartment_statistic_Groups);
        }

        private void onchangeDepartment_statistic_Groups(object sender, SqlNotificationEventArgs e)
        {
            if (e.Info != SqlNotificationInfo.Invalid)
                dtvDepartment_statistic_Groups();
        }
        ...
        public void dtvDepartment_statistic_Students()
        {
            SqlDependency dependency = new SqlDependency(command);
            SqlDependency.Start(Registry_Class.sqlConnection.ConnectionString);
            dependency.OnChange += onchangeDepartment_statistic_Students;
            dtvFill(vdtDepartment_statistic_Students, qrvDepartment_statistic_Students);
        }

        private void onchangeDepartment_statistic_Students(object sender, SqlNotificationEventArgs e)
        {
            if (e.Info != SqlNotificationInfo.Invalid)
                dtvDepartment_statistic_Students();
        }
        ...
        public void dtvGroup_statistics_in_studing()
        {
            SqlDependency dependency = new SqlDependency(command);
            SqlDependency.Start(Registry_Class.sqlConnection.ConnectionString);
            dependency.OnChange += onchangeGroup_statistics_in_studing;
            dtvFill(vdtGroup_statistics_in_studing, qrvGroup_statistics_in_studing);
        }

        private void onchangeGroup_statistics_in_studing(object sender, SqlNotificationEventArgs e)
        {
            if (e.Info != SqlNotificationInfo.Invalid)
                dtvGroup_statistics_in_studing();
        }
        ...
        public void dtvGroup_academic_performance()
        {
            SqlDependency dependency = new SqlDependency(command);
            SqlDependency.Start(Registry_Class.sqlConnection.ConnectionString);
            dependency.OnChange += onchangeGroup_academic_performance;
            dtvFill(vdtGroup_academic_performance, qrvGroup_academic_performance);
        }

        private void onchangeGroup_academic_performance(object sender, SqlNotificationEventArgs e)
        {
            if (e.Info != SqlNotificationInfo.Invalid)
                dtvGroup_academic_performance();
        }
        ...
        public void dtvStudent_Marks()
        {
            SqlDependency dependency = new SqlDependency(command);
            SqlDependency.Start(Registry_Class.sqlConnection.ConnectionString);
            dependency.OnChange += onchangeStudent_Marks;
            dtvFill(vdtStudent_Marks, qrvStudent_Marks);
        }

        private void onchangeStudent_Marks(object sender, SqlNotificationEventArgs e)
        {
            if (e.Info != SqlNotificationInfo.Invalid)
                dtvStudent_Marks();
        }
        ...
        public void dtvEmployee_Burden()
        {
            SqlDependency dependency = new SqlDependency(command);
            SqlDependency.Start(Registry_Class.sqlConnection.ConnectionString);
            dependency.OnChange += onchangeEmployee_Burden;
            dtvFill(vdtEmployee_Burden, qrvEmployee_Burden);
        }

        private void onchangeEmployee_Burden(object sender, SqlNotificationEventArgs e)
        {
            if (e.Info != SqlNotificationInfo.Invalid)
                dtvEmployee_Burden();
        }
